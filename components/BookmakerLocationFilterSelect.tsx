...
export type BookmakerLocationFilter =
  | { type: "all" }
  | { type: "region"; region: RegionSlug }
  | { type: "country"; region: RegionSlug; country: CountrySlug };

type BookmakerLocationFilterSelectProps = {
  value: BookmakerLocationFilter;
  onChange: (value: BookmakerLocationFilter) => void;
};
...
const COUNTRY_CODE_TO_ROUTE = COUNTRY_OPTIONS.reduce<
  Partial<Record<string, CountryRouteMatch>>
>((acc, country) => {
  if (country.flagCode) {
    acc[country.flagCode.toUpperCase()] = {
...
function LocationFlag({
  ...
  if (countryCode) {
    return (
      <span
        className={`fi fi-${countryCode.toLowerCase()} inline-block h-3 w-4 overflow-hidden rounded-sm`}
        aria-hidden="true"
        title={label}
      />
    );
...
export default function BookmakerLocationFilterSelect({
  value,
  onChange,
}: BookmakerLocationFilterSelectProps) {
  ...
  function getSelectedLocation() {
    if (value.type === "country") {
      const country = COUNTRY_OPTIONS.find((c) => c.slug === value.country);

      return {
        label: COUNTRY_LABELS[value.country],
    ...
    if (value.type === "region") {
      const region = REGION_OPTIONS.find((r) => r.slug === value.region);

      return {
        label: REGION_LABELS[value.region],
  ...
  function selectLocation(region?: RegionSlug, country?: CountrySlug) {
    setOpen(false);
    setQuery("");
    setLocationError("");
  ...
  async function handleUseMyLocation() {
    ...
    try {
      ...
      selectLocation(match.region, match.country);
    } catch (error) {
        ...
        <div className="absolute right-0 z-30 mt-2 w-full overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-lg">
          ...
          <div className="max-h-96 overflow-y-auto p-2">
            <div className="mb-2">
              ...
              {locationError && (
                <div className="px-3 pt-1 text-xs text-rose-600">
                  {locationError}
                </div>
              )}
            </div>
            ...
            {filteredRegions.map((region) => (
              <button
                key={region.slug}
                type="button"
                onClick={() => selectLocation(region.slug)}
              ...
              <div>
                ...
                {filteredCountries.map((country) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      selectLocation(country.region, country.slug)
                    }
